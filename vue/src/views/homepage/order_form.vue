...
    </div>
    ...
    <div class="hight-r"></div>
		...
		<div class="pro">
			...
			<div class="bar">
				<span>商品租金</span>
				<span class="org">￥{{price*buy_num}}</span>
			</div>
			<div class="bar">
				<span>押金</span>
				<span class="org">+￥{{depositprice*buy_num}}</span>
			</div>
			<div class="bar">
				<span>优惠</span>
		...
		<div class="btn">
			<div class="sum">
				<span>合计：<span class="org">￥{{sunm + (price*buy_num) + (depositprice*buy_num)-yhj_price}}</span></span>
			</div>
...
    },
      ...
      this.shous = true;
		...
		ChangeMenu: function(index) {
			// debugger
			let self = this;
			self.yhj_index = index;
			self.yhj_price = self.press[self.yhj_index].price;
			self.yhj_id = self.press[self.yhj_index].id;
			console.log(1111,self.yhj_price ,self.yhj_id)
		},
		// 优惠卷
		discounts: function() {
     let self = this;
			...
			self.httpGet(url,function(res) {
				if (res.code == 1) {
					self.press = res.data;
					if(self.press.length>0)
					{
						self.curYUJ = self.press[0].price;
					}
				} else {
					self.$dialog.toast({
						mes: res.msg,
      ...
      self.httpGet(url,function(res) {
        if (res.code == 1) {
					wx.config({
						...
						timestamp: 1556086943, 
						nonceStr: "l8qOZPMsR1ACEXAs", 
  ...
  created() {
		...
		this.depositprice = this.$route.query.depositprice;
